...
void LoadFace(const char* bufferTGA, int width, GLenum type, int bytePP, char* face)
{
     ...
     int x_start = pos_x * block_width * bytePP;
     int y_start = pos_y * block_width;
     for (size_t i = 0; i < block_width; i++) {
       int y_offset = (y_start + i) * width * bytePP;
       memcpy(&face[i * block_width * bytePP], &bufferTGA[y_offset + x_start],  block_width * bytePP);
     }
